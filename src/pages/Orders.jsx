...
export default function Orders() {
  ...
  const handleTrackOrder = async (e) => {
    ...
    // We reuse logic but add phone verification check inside the modal render
    loadOrderDetails(Number(searchId));
    // We set a flag or just use the existing selectedOrder mechanism
    // But since loadOrderDetails sets 'orderDetails', we need to check if we should ALSO set 'selectedOrder'
    // Actually, 'selectedOrder' is used for the modal visibility.
    // Let's mock a selectedOrder object so the modal opens.
    setSelectedOrder({ id: Number(searchId) });
  };
  ...
  return (
    ...
    >
      ...
      {/* Track Order Section */}
      <div className="mb-12 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-gray-700">
        ...
        <form onSubmit={handleTrackOrder} className="flex flex-col md:flex-row gap-4">
           <div className="flex-1">
             <input 
               type="number"
               placeholder={language === "ar" ? "رقم الطلب (Order ID)" : "Order ID"}
               className="w-full px-4 py-3 rounded-xl border dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:ring-2 focus:ring-accent outline-none"
               value={searchId}
           ...
           <button 
             type="submit"
             className="px-8 py-3 bg-accent hover:bg-yellow-600 text-white rounded-xl font-bold transition-all shadow-lg hover:shadow-accent/20"
           >
         ...
         <p className="text-xs text-gray-500 mt-2">
            {language === "ar" 
              ? "إذا لم يظهر طلبك في القائمة أدناه (بسبب تغيير الجهاز)، يمكنك البحث برقم الطلب." 
              : "If your order doesn't appear below (due to verified device change), search by Order ID."}
         </p>
      </div>
